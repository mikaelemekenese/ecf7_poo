...
<div class="container" style="margin-top:68px;">
    ...
    <nav class="level" style="margin-bottom:0;">
        ...
        <div class="level-center">
            <div class="level-item">
                <?php $id = $_GET['id']; $actor = Actor::findById($id); ?>
                <h1 class="title is-centered">Movies with : <span style="color:white;font-weight:bold;"><?php echo ucwords(strtolower($actor['actorFirstName'])) ?> <?php echo ucwords(strtolower($actor['actorLastName'])) ?></span></h1>
            </div>
        </div>
    </nav>
    ...
    <table class="table table-dark table-striped has-text-primary-light">
        ...
        <tbody>
        <?php $id = $_GET['id']; $films = Film::findByActor($id); foreach($films as $film) : ?>
            <tr class="text-center">
