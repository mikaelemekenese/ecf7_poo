...
class Actor extends Database 
{
    ...
    {
        $actor = self::query("SELECT a.actor_id AS actor_id, a.first_name AS actorFirstName, a.last_name AS actorLastName FROM actor AS a
    ...
    public static function findByFilm(int $id) 
    {
        $actor = self::query("SELECT a.actor_id AS actor_id, a.first_name AS actorFirstName, a.last_name AS actorLastName FROM actor AS a
            LEFT JOIN film_actor AS fa ON a.actor_id = fa.actor_id
            LEFT JOIN film AS f ON f.film_id = fa.film_id
